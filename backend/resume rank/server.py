...
def levenshtein_similarity(s1, s2):
    ...
    larger_length = max(len(s1), len(s2))
    
    # Avoid division by zero
    if larger_length == 0:
        return 100 if s1 == s2 else 0  

    similarity = (1 - distance / larger_length) * 100
...
def Matching(resume_data, job_desc):
    try:
        ...
        # --- Safely access job required experience ---
        job_required_experience = dic_jd.get('EXPERIENCE', [])
        if job_required_experience:
            job_required_experience = job_required_experience[0].split(' ')[0]
        else:
            job_required_experience = None  # Handle missing experience case
        ...
        if resume_workedAs and jd_post:
            ...
            for resume_pos in resume_workedAs:
                for jd_pos in jd_post:
                    similarity = levenshtein_similarity(resume_pos.lower(), jd_pos.lower())
        ...
        if resume_experience_years and job_required_experience:
            if job_required_experience is not None:
                job_required_experience = float(job_required_experience)
                print(job_required_experience)
            else:
                job_required_experience = 0

            print(job_required_experience)
            if len(resume_experience_years) > 0 and job_required_experience > 0:
                if abs(resume_experience_years[0] - float(job_required_experience)) <= 2:
                    experience_similarity = 0.7
                elif resume_experience_years[0] > float(job_required_experience):
        ...
        if resume_skills and job_description_skills:
            ...
            for resume_skill in resume_skills:
                for job_skill in job_description_skills:
                    if levenshtein_similarity(resume_skill.lower(), job_skill.lower()) >= 50:
                        matches += 1
                        break
        ...
        overall_similarity = (jdpost_similarity * 0.1 + experience_similarity * 0.2 + skills_similarity * 0.7) * 100
